...
import { HETZNER_ASN, SERVER_HOST, SERVER_IP, PORT, STUB_TRACEROUTE, ADMIN_PASSWORD } from './env.js'
import express from 'express'
import ejs from 'ejs'
import { AsyncRouter } from 'express-async-router'
import fs from 'node:fs'
import { nanoid } from 'nanoid'
import { ktrVersion } from './ktr.js'
import { getKtr } from './ktr-pool.js'
...
router.get('/', async (req, res) => {
	...
	if (!STUB_TRACEROUTE) {
		...
		try {
			const { streamId, html } = renderTracerouteUpdate({
				...
				pageGlobals, templates, lastStreamId, hetznerInfo
			})
			res.write(html)
	...
	if (STUB_TRACEROUTE) {
		...
		const { html } = renderTracerouteUpdate({ update: _testUpdate, pageGlobals, templates, lastStreamId: null, hetznerInfo })
		res.write(html)
		res.end(ejs.render(afterSplit, { pageGlobals }))
	}
})
